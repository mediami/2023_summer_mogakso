...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attention = torch.sigmoid(self.conv(stacked))
        return attention
...
def create_resnet(model_name, num_classes, cbam=False):
...
class CBAMBottleNeck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, norm_layer, downsample=None, groups=1, base_width=64,
        ...
        self.cbam = CBAMBlock(self.out_channels) if cbam else nn.Identity()
    ...
    if __name__ == '__main__':
        ...
        model = create_resnet('resnet34', 100, cbam=True)
        y = model(x)
